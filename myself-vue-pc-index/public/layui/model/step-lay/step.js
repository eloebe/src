...
layui.define(['layer', 'carousel','jquery'], function (exports) {
    var $ = layui.jquery;
        ...
        for (var i = 0; i < stepItems.length; i++) {
            ...
            if (i < (stepItems.length - 1)) {
                if (i < postion) {
					...
					stepDiv += '<div class="step-item-tail"><i class="layui-icon layui-icon-right" style="font-size:25px;color:#43555d;"></i></div>';
                } else {
                    // stepDiv += '<div class="step-item-tail"><i class=""></i></div>';
					stepDiv += '<div class="step-item-tail"><i class="layui-icon layui-icon-right" style="font-size: 25px;color:#c3cacb;"></i></div>';
                }
            }
    ...
    var step = {
        ...
        render: function (param) {
			...
			// add by myself
				// $('.lay-step').css('border-top','1px solid #E1E8EC');
				$('.step-item-head-active').next('.step-item-main').find('.step-item-main-title').css("font-weight","bold");
            //监听轮播切换事件
	...
	// layui.link('/layui/lay/modules/step.css');
	// layui.link('step.css');
    exports('step', step);
});
